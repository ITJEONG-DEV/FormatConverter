...
updater.py
----------
GitHub Releases 기반 자동 업데이트 (UI 독립적인 순수 로직).

흐름:
  1) check_update(현재버전)  -> 최신 릴리스가 더 높으면 정보 dict 반환, 아니면 None
...
진단 로그: %TEMP%/FormatConverter_update.log
"""
...
REPO = "ITJEONG-DEV/FormatConverter"
...
# 도우미 스크립트 진단 로그(고정 위치)
_LOG_PATH = os.path.join(tempfile.gettempdir(), "FormatConverter_update.log")
...
def _wait_block(pid: int, log: str) -> str:
    """현재 프로세스가 종료될 때까지 기다리는 PowerShell 코드."""
    return _ps_lines(
...
def _lite_script(pid: int, cur: str, new: str, log: str) -> str:
    # 실행 중인 exe는 직접 덮어쓸 수 없으므로 rename 후 새 exe를 복사한다.
    return _ps_lines(
...
def _launch_helper(script_text: str) -> None:
    """도우미 PowerShell 스크립트를 파일로 쓰고 숨김 실행."""
    helper_dir = tempfile.mkdtemp(prefix="fc_helper_")
    ps1 = os.path.join(helper_dir, "update.ps1")
    # UTF-8 BOM 이어야 Windows PowerShell 5.1이 비ASCII 경로를 안정적으로 읽는다.
...
def download_and_apply(
    ...
    """
    최신 자산을 내려받고 도우미 PowerShell 스크립트를 실행한다.
    호출부는 이 함수 반환 후 앱을 종료해야 도우미가 파일을 교체할 수 있다.
    """
    if kind == "dev":
    ...
    if kind == "lite":
        new_exe = _find(extract, "FormatConverter-lite.exe")
        if not new_exe:
